...
class Global_Scene_CAM_Path_NFDecoder(CAM):
    ...
    def __init__(self, device, agent_embed_dim, nfuture, att_dropout, velocity_const, num_candidates, att=True):
        ...
        self.cnn_model = NewModelShallowCNN()
        self.context_fusion = ContextFusion(hidden_size=agent_embed_dim)
        self.crossmodal_dynamic_decoder = CrossModalPathDecoder(decoding_steps=1, velocity_const=velocity_const, att=att)
    ...
    def infer(self, tgt_trajs, agent_encodings_, episode_idx, decode_start_vel, decode_start_pos, num_past_agents, scene_or_scene_encoding, goal_encoding):
        ...
        z, mu, sigma = self.crossmodal_dynamic_decoder.infer(tgt_trajs[:,:-1,:].reshape(-1, 1, 2), context_encoding, decode_start_vel, decode_start_pos, global_scene_encoding_, episode_idx, goal_encoding)
        return z, mu, sigma, agent_encodings_, (local_scene_encoding_, global_scene_encoding_)