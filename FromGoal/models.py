...
from MATF.models import *
from FromGoal.model_utils import *
...
# GlobalScene + LocalScene + CAM + NF & AttGlobalScene + LocalScene + CAM + NF for GOAL generation
class Global_Scene_CAM_Goal_NFDecoder(CAM):
    """
    Model2: Cross-agent Attention & Normalizing Flow Decoder
    """
    def __init__(self, device, agent_embed_dim, nfuture, att_dropout, velocity_const, num_candidates, att=True):

        super(Global_Scene_CAM_Goal_NFDecoder, self).__init__(device, agent_embed_dim, nfuture, att_dropout)
        ...
        self.cnn_model = NewModelShallowCNN()
        self.context_fusion = ContextFusion(hidden_size=agent_embed_dim)
        self.crossmodal_dynamic_decoder = CrossModalGoalDecoder(decoding_steps=1, velocity_const=velocity_const, att=att)
    ...
    def forward(self, src_trajs_or_src_encoding, src_lens, future_agent_masks, episode_idx, decode_start_vel, decode_start_pos, num_past_agents, scene_or_scene_encoding, agent_encoded=False, scene_encoded=False):
        ...
        x_t, mu_t, sigma_t, goal_encoding = self.crossmodal_dynamic_decoder(z_t, x_flat, context_encoding, dx, x_prev, global_scene_encoding_, episode_idx)
        ...
        x = x_t.reshape(num_agents, self.num_candidates, self.decoding_steps, 2) # x: Na X Nc X Td X 2	
        z = z.reshape(num_agents, self.num_candidates, self.decoding_steps*2)	
        mu = mu_t.reshape(num_agents, self.num_candidates, self.decoding_steps, 2) # mu: Na X Nc X Td X 2	
        sigma = sigma_t.reshape(num_agents, self.num_candidates, self.decoding_steps, 2, 2) # sigma: Na X Nc X Td X 2 X 2	

        return x, z, mu, sigma, goal_encoding
    ...
    def infer(self, tgt_trajs, src_trajs, src_lens, future_agent_masks, episode_idx, decode_start_vel, decode_start_pos, num_past_agents, scene):
        ...
        z, mu, sigma = self.crossmodal_dynamic_decoder.infer(tgt_trajs[:,-1,:].reshape(-1, 1, 2), context_encoding, decode_start_vel, decode_start_pos, global_scene_encoding_, episode_idx)
        return z, mu, sigma, agent_encodings_, (local_scene_encoding_, global_scene_encoding_)

# AttGlobalScene + CAM + NF for GOAL generation (no local scene feature)
class Only_Global_Scene_CAM_Goal_NFDecoder(CAM):
    """
    Model2: Cross-agent Attention & Normalizing Flow Decoder
    """
    def __init__(self, device, agent_embed_dim, nfuture, att_dropout, velocity_const, num_candidates, att=True):

        super(Only_Global_Scene_CAM_Goal_NFDecoder, self).__init__(device, agent_embed_dim, nfuture, att_dropout)
        ...
        self.decoding_steps = 1    
        self.num_candidates = num_candidates
        self.mlp_no_local = nn.Sequential(
            nn.Linear(agent_embed_dim, 50),
            nn.Softplus(),
            nn.Linear(50, 50),
            nn.Softplus(),
    ...
    def forward(self, src_trajs_or_src_encoding, src_lens, future_agent_masks, episode_idx, decode_start_vel, decode_start_pos, num_past_agents, scene_or_scene_encoding, agent_encoded=False, scene_encoded=False):
        ...
        if agent_encoded:
            ...
            agent_encodings = self.encoder(src_trajs_or_src_encoding, src_lens, future_agent_masks, num_past_agents) # (Ad X Dim)
        context_encoding = self.mlp_no_local(agent_encodings)	
        ...
        x_t, mu_t, sigma_t = self.crossmodal_dynamic_decoder(z_t, x_flat, context_encoding, dx, x_prev, global_scene_encoding_, episode_idx)
...
# AttGlobalScene + LocalScene + CAM + NF for PATH generation
class Global_Scene_CAM_Path_NFDecoder(CAM):
    """
    Generate only one path for one goal
    """
    def __init__(self, device, agent_embed_dim, nfuture, att_dropout, velocity_const, num_candidates, att=True):

        super(Global_Scene_CAM_Path_NFDecoder, self).__init__(device, agent_embed_dim, nfuture, att_dropout)

        self.cnn_model = NewModelShallowCNN()
        self.context_fusion = ContextFusion(hidden_size=agent_embed_dim)
        self.crossmodal_dynamic_decoder = CrossModalPathDecoder(decoding_steps=5, velocity_const=velocity_const, att=att)

        self.interpolator = Bilinear_Interpolation()
        self.decoding_steps = 5
        self.num_candidates = 1

    def forward(self, src_trajs_or_src_encoding, src_lens, future_agent_masks, episode_idx, decode_start_vel, decode_start_pos, num_past_agents, scene_or_scene_encoding, goal, goal_encoding):
        """
        input shape
        src_trajs_or_src_encoding:
          A x Te x 2 if src_trajs
        ...
        episode_idx = episode_idx.repeat_interleave(self.num_candidates)
        agent_encodings = agent_encodings.repeat_interleave(self.num_candidates, dim=0)
        goal_encoding = goal_encoding.repeat_interleave(self.num_candidates, dim=0)
        decode_start_vel = decode_start_vel.repeat_interleave(self.num_candidates, dim=0)	
        decode_start_pos = decode_start_pos.repeat_interleave(self.num_candidates, dim=0)
        ...
        x_t, mu_t, sigma_t, goal_pack = self.crossmodal_dynamic_decoder(z_t, x_flat, context_encoding, dx, x_prev, global_scene_encoding_, episode_idx, goal, goal_encoding)
        ...
        ###############################################
        x_g = goal_pack[0].reshape(num_agents, self.num_candidates, self.decoding_steps, 2)
        x = x_t.reshape(num_agents, self.num_candidates, self.decoding_steps, 2) # x: Na X Nc X Td X 2	
        z = z.reshape(num_agents, self.num_candidates, self.decoding_steps*2)	
        mu = mu_t.reshape(num_agents, self.num_candidates, self.decoding_steps, 2) # mu: Na X Nc X Td X 2	
        sigma = sigma_t.reshape(num_agents, self.num_candidates, self.decoding_steps, 2, 2) # sigma: Na X Nc X Td X 2 X 2	

        return x, z, mu, sigma, x_g

    def infer(self, tgt_trajs, agent_encodings_, episode_idx, decode_start_vel, decode_start_pos, num_past_agents, scene_or_scene_encoding, goal_encoding):
        """
        input shape
        tgt_trajs: Ad X Td X 2
        src_trajs: Ae X Te X 2
        ...
        agent_encodings = agent_encodings_.unsqueeze(dim=1) # (B X 1 X Dim)
        # agent_encodings = agent_encodings.expand(-1, self.decoding_steps, -1) # [B X T X Dim]
        agent_encodings = agent_encodings.expand(-1, 1, -1) # [B X T X Dim]

        context_encoding, _  = self.context_fusion(agent_encodings, interpolated_feature) # [A X Td X 50]	

        z, mu, sigma, goal_pack = self.crossmodal_dynamic_decoder.infer(tgt_trajs, context_encoding, decode_start_vel, decode_start_pos, global_scene_encoding_, episode_idx, goal_encoding)
        return z, mu, sigma, agent_encodings_, (local_scene_encoding_, global_scene_encoding_), goal_pack