...
from Paths.models import Global_Scene_CAM_Goal_NFDecoder, Only_Global_Scene_CAM_Goal_NFDecoder, Global_Scene_CAM_Path_NFDecoder
from Paths.utils import ModelTrainer, ModelTest
...
def train(args):
    ...
    if args.optimizer == 'adam':
        optimizer = torch.optim.Adam(list(goal_model.parameters()) + list(path_model.parameters()), lr=args.learning_rate, weight_decay=1e-4)
    ...
    # Training Runner
    trainer = ModelTrainer(goal_model, path_model, train_loader, valid_loader, optimizer, exp_path, args, device, ploss_criterion, path_ploss_criterion)
...
class Visualizer:
    def __init__(self, goal_model, path_model, ploss_criterion, root='../datasets/nuscene/v1.0-mini', version='v1.0-mini'):
        self.root = root
        ...
        with torch.no_grad():
            ...
            for b, batch in tqdm(enumerate(dataloader), total=len(dataloader) // batch_size, desc='predict'):
                ...
                for candi in range(gen_goals.size(1)):
                    ...
                    z_, mu_, sigma_, motion_encoding_, scene_encoding_ = self.path_model.infer(tgt_trajs + perterb,
                                                                                            motion_encoding_,
                                                                                            episode_idx,
                                                                                            decode_start_vel,
                                                                                            decode_start_pos,
                    ...
                    # Prior Loss (p loss)
                    gen_trajs, z, mu, sigma = self.path_model(motion_encoding_, src_lens, agent_tgt_three_mask,
                                                                episode_idx, decode_start_vel, decode_start_pos,
...
def visualize(args):
    ...
    viz = Visualizer(goal_model, path_model, ploss_criterion, root='{}/original_small/{}'.format(load_dir, version), version=version)
