    ...
    def train_single_epoch(self):
        ...
        for b, batch in enumerate(self.train_loader):
            ...
            if self.flow_based_decoder:
                ...
                elif self.model_type == 'AttGlobal_Scene_CAM_Goal_NFDecoder':
                    z_, mu_, sigma_, motion_encoding_, scene_encoding_ = self.model.infer(future_agents_traj + perterb,
                                                                                          past_agents_traj,
                                                                                          past_agents_traj_len,
                                                                                          future_agent_masks,
                                                                                          ...
                                                                                          decode_start_pos,
                                                                                          num_past_agents, scene_images)
                elif self.model_type == 'Only_AttGlobal_Scene_CAM_Goal_NFDecoder':
                    z_, mu_, sigma_, motion_encoding_, scene_encoding_ = self.model.infer(future_agents_traj + perterb,
                                                                                          past_agents_traj,
                                                                                          past_agents_traj_len,
                                                                                          future_agent_masks,
                ...
                if self.beta != 0.0:
                    if self.ploss_type == 'mseloss':
                        ...
                        ploss = self.ploss_criterion(gen_trajs, future_agents_traj[:,-1,:].unsqueeze(1))
                    else:
            ...
            rs_error3 = ((gen_trajs.squeeze(2) - future_agents_traj[:,-1,:].unsqueeze(1)) ** 2).sum(dim=-1).sqrt_()  # A X candi X T X 2 >> A X candi X T
    ...
    def inference(self):
        ...
        with torch.no_grad():
            ...
            for b, batch in enumerate(self.valid_loader):
                ...
                rs_error3 = ((gen_trajs.squeeze(2) - future_agents_traj[:,-1,:].unsqueeze(1)) ** 2).sum(dim=-1).sqrt_()
