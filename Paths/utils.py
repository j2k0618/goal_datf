...
class ModelTrainer:
    ...
    def train_single_epoch(self):
        ...
        for b, batch in enumerate(self.train_loader):
            ...
            z_, mu_, sigma_, motion_encoding_, scene_encoding_ = self.path_model.infer(future_agents_traj + perterb,
                                                                                    motion_encoding_,
                                                                                    episode_idx, decode_start_vel,
                                                                                    decode_start_pos,
            ...
            # Prior Loss (p loss)
            gen_trajs, z, mu, sigma = self.path_model(motion_encoding_, past_agents_traj_len, agent_tgt_three_mask,
                                                    episode_idx, decode_start_vel, decode_start_pos,
                                                    num_past_agents, scene_encoding_, future_agents_traj[:,-1,:], goal_encoding)

            # print(gen_trajs.size())
            # print(future_agents_traj[:,:-2,:].size())
            if self.beta != 0.0:
                if self.path_ploss_type == 'mseloss':
                    # ploss = self.ploss_criterion(gen_trajs, past_agents_traj)
                    ploss = self.path_ploss_criterion(gen_trajs, future_agents_traj[:,:-1,:]) + self.ploss_criterion(episode_idx, gen_trajs, log_prior, -15.0)
                else:
                    ploss = self.path_ploss_criterion(episode_idx, gen_trajs, log_prior, -15.0)
                
            else:
                ploss = torch.zeros(size=(1,), device=self.device)

            batch_path_ploss = ploss.mean()
            batch_loss += (batch_path_qloss + self.beta * batch_path_ploss) * self.path_weight
