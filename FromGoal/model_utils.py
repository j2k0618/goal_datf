...
class CrossModalGoalDecoder(nn.Module):
    """
    Dynamic Decoder for R2P2 RNN
    """
    def __init__(self, image_dim=32, hidden_size=150, context_dim=50, decoding_steps=1, velocity_const=0.5, att=True):
        super(CrossModalGoalDecoder, self).__init__()
        self.velocity_const = velocity_const
        self.decoding_steps = decoding_steps
        # self.lstm = nn.LSTM(decoding_steps*2, hidden_size, 1)        
        self.gru = nn.GRU(input_size=decoding_steps*2, hidden_size=hidden_size, num_layers=1)        
        self.mlp_goal = nn.Sequential(
            nn.Linear(2, 50),
            nn.ReLU(),
            nn.Linear(50, hidden_size) # DON'T USE ACTIVATION AT THE TOP MOST LAYER
        )
        self.crossmodal_attention = CrossModalAttention(encoder_dim=image_dim, decoder_dim=hidden_size, attention_dim=hidden_size, att=att) # map2agent attention
        self.mlp = nn.Sequential(
    ...
    def infer(self, x, static, init_velocity, init_position, global_scene_encoding, episode_idx):
        ...
        # Detect prediction length
        T = x.size(1) # T: Prediction Time Length
        assert T==1 # T should be 1 since it is the goal(final destination) 

        # Build the state differences for each timestep
        # dx = x[:, 1:, :] - x[:, :-1, :] # B X (T-1) X 2
        # dx = torch.cat((init_velocity.unsqueeze(1), dx), dim=1) # B X T X 2
        dx = init_velocity.unsqueeze(1)
        
        # Build the previous states for each timestep
        # x_prev = x[:, :-1, :] # B X (T-1) X 2
        # x_prev = torch.cat((init_position.unsqueeze(1), x_prev), dim=1) # B X T X 2
        x_prev = init_position.unsqueeze(1) # B X T(=1) X 2
        ...
        # Get attn for all timesteps and agents
        # dynamic_encoding, _ = self.gru(x_flat) # dynamic_encoding: T X B X 150(hidden size)
        # dynamic_encoding = dynamic_encoding.transpose(0, 1)

        ##########################################
        # MLP for "GOAL=Endpoint" generation 
        ##########################################
        dynamic_encoding = self.mlp_goal(x_flat) # dynamic_encoding: B X 150(hidden size)

        dynamic_encoding_ = dynamic_encoding # Since T is 1(only for 1 goal)
        dynamic_encoding = dynamic_encoding.unsqueeze(1) # B X T X 150
        episode_idx_ = episode_idx.repeat_interleave(T)
    ...
    def forward(self, z, x_flat, static, dx, x_prev, global_scene_encoding, episode_idx):
        '''
        input shape
        z: A X 2
        x_flat: A X 60
        ...
        # Detect dynamic batch size
        batch_size = static.size(0)
        num_agent = int(batch_size/6)
        # print(num_agent)
        # print(batch_size)
        
        # Unroll a step
        dynamic_encoding = self.mlp_goal(x_flat.unsqueeze(0))[-1]
        goal_encoding = self.mlp_goal(x_flat[:num_agent].unsqueeze(0))[-1]


        att_scene, alpha = self.crossmodal_attention(global_scene_encoding, dynamic_encoding, episode_idx)
        att_goal_scene, _ = self.crossmodal_attention(global_scene_encoding, goal_encoding, episode_idx[:num_agent])

        # Concat the dynamic and static encodings
        dynamic_static = torch.cat((dynamic_encoding, static, att_scene), dim=-1) # B X 232
        goal_static = torch.cat((goal_encoding, static[:num_agent], att_goal_scene), dim=-1) # B X 232
        ...
        return x, mu, sigma, goal_static

class CrossModalPathDecoder(nn.Module):
    """
    Dynamic Decoder for R2P2 RNN
    """
    def __init__(self, image_dim=32, hidden_size=150, context_dim=50, decoding_steps=5, velocity_const=0.5, att=True, goal_enc_dim=232):
        super(CrossModalPathDecoder, self).__init__()
        self.velocity_const = velocity_const
        self.decoding_steps = decoding_steps
        # self.lstm = nn.LSTM(decoding_steps*2, hidden_size, 1)        
    ...
    def infer(self, x, static, init_velocity, init_position, global_scene_encoding, episode_idx, goal_encoding):
        '''
        input shape
        x: A X Td X 2 // future trj coordindates
        static: A X Td X 50 // past trj encoding
        ...
        # Detect prediction length
        T = x.size(1) # T: Prediction Time Length
        T = T-1 # Exclude the endpoint

        # Build the state differences for each timestep
        # dx = x[:, 1:-1, :] - x[:, :-2, :] # B X (T-1) X 2
        # dx = torch.cat((init_velocity.unsqueeze(1), dx), dim=1) # B X T X 2
        dx = init_velocity.unsqueeze(1)
        
        # Build the previous states for each timestep
        x_goal = x[:, -1, :].unsqueeze(1) # B X 1 X 2
        x_prev = x[:, :-2, :] # B X (T-1) X 2
        x_prev = torch.cat((init_position.unsqueeze(1), x_prev), dim=1) # B X T X 2
        x_input = torch.cat((init_position.unsqueeze(1), x_goal), dim=1) # B X 2 X 2

        # Build the flattend & zero padded previous states for GRU input
        x_flat = x_input.reshape((batch_size, -1)) # B X 2 X 2 >> B X (2*2)

        ##########################################
        # MLP for "PATH" generation 
        ##########################################  
        dynamic_encoding = self.mlp_path(x_flat) # dynamic_encoding: B X 150 (hidden size)

        dynamic_encoding_ = dynamic_encoding.reshape(batch_size * 1, -1)
        dynamic_encoding = dynamic_encoding.reshape(batch_size, 1, -1) # B X 1 X 150
        episode_idx_ = episode_idx.repeat_interleave(1)

        att_scenes, alpha = self.crossmodal_attention(global_scene_encoding, dynamic_encoding_, episode_idx_)
        att_scenes = att_scenes.reshape(batch_size, 1, -1)

        # Concat the dynamic and static encodings
        dynamic_static = torch.cat((dynamic_encoding, static, att_scenes, goal_encoding.unsqueeze(1)), dim=-1) # B X T X 200
        # dynamic_static = torch.cat((dynamic_encoding, static, att_scenes), dim=-1) # B X T X 200

        # 2-layer MLP from start
        output = self.mlp(dynamic_static) # B X (T X 6)
        output = output.reshape(batch_size, self.decoding_steps, 6) # B X T X 6
        mu_hat = output[:, :, :2] # [B X T X 2]
        sigma_hat = output[:, :, 2:].reshape((batch_size, T, 2, 2)) # [B X T X 2 X 2]

        # 2-layer MLP from goal
        output_g = self.mlp_from_goal(dynamic_static) # B X (T X 6)
        output_g = output_g.reshape(batch_size, self.decoding_steps, 6) # B X T X 6
        mu_hat_g = output_g[:, :, :2] # [B X T X 2]
        sigma_hat_g = output[:, :, 2:].reshape((batch_size, T, 2, 2)) # [B X T X 2 X 2]

        # verlet integration
        mu = init_position.unsqueeze(1) + mu_hat  + self.velocity_const * dx
        mu_g = x_goal - mu_hat_g
        ...
        # solve  Z = inv(sigma) * (X-mu)
        X_mu = (x[:,:-1] - mu).unsqueeze(-1) # B X T X 2 X 1
        z, _ = X_mu.solve(sigma) # B X T X 2 X 1
        z = z.squeeze(-1) # B X T X 2


        # z, mu, sigma from goal
        # New matrix Exponential
        b = sigma_hat_g[:, :, 0, 1] + sigma_hat_g[:, :, 1, 0]
        apd_2 = sigma_hat_g[:, :, 0, 0] + sigma_hat_g[:, :, 1, 1] # (a+d) / 2
        amd_2 = sigma_hat_g[:, :, 0, 0] - sigma_hat_g[:, :, 1, 1] # (a-d) / 2
        delta = torch.sqrt(amd_2 ** 2 + b ** 2)
        sinh = torch.sinh(delta)
        cosh = torch.cosh(delta)

        sigma_g = torch.zeros_like(sigma_hat_g)

        tmp1 = sinh / delta
        tmp2 = amd_2 / tmp1

        sigma_g[:, :, 0, 0] = cosh + tmp2
        sigma_g[:, :, 0, 1] = b * tmp1
        sigma_g[:, :, 1, 0] = sigma_g[:, :, 0, 1]
        sigma_g[:, :, 1, 1] = cosh - tmp2
        sigma_g *= torch.exp(apd_2).unsqueeze(-1).unsqueeze(-1)

        # solve  Z = inv(sigma) * (X-mu)
        X_mu = (x[:,:-1] - mu_g).unsqueeze(-1) # B X T X 2 X 1
        z_g, _ = X_mu.solve(sigma_g) # B X T X 2 X 1
        z_g = z_g.squeeze(-1) # B X T X 2


        return z, mu, sigma, (z_g, mu_g, sigma_g)

    def forward(self, z, x_flat, static, dx, x_prev, global_scene_encoding, episode_idx, goal, goal_encoding):
        '''
        input shape
        z: A X 2
        x_flat: A X 60
        ...
        x_flat = torch.cat((x_flat, goal), dim=1)

        # Unroll a step
        dynamic_encoding = self.mlp_path(x_flat.unsqueeze(0))[-1]

        # print(global_scene_encoding.size())
        # print(dynamic_encoding_.size())
        # print(dynamic_encoding_.size())
        att_scene, alpha = self.crossmodal_attention(global_scene_encoding, dynamic_encoding, episode_idx)

        # Concat the dynamic and static encodings
        # print(att_scene.size())
        # print(dynamic_encoding.size())
        # print(static.size())
        # dynamic_static = torch.cat((dynamic_encoding, static, att_scene), dim=-1) # Need to be B X 200
        ...
        output = output.reshape(batch_size, self.decoding_steps, 6) # B X T X 6
        mu_hat = output[:, :, :2] # B X T X 2
        sigma_hat = output[:, :, 2:].reshape((batch_size, self.decoding_steps, 2, 2)) # B X T X 2 X 2

        x_prev = x_prev.unsqueeze(1)
        mu = x_prev + mu_hat  + self.velocity_const * dx.unsqueeze(1)
        ...
        output_g = output_g.reshape(batch_size, self.decoding_steps, 6) # B X T X 6
        mu_hat_g = output_g[:, :, :2] # B X T X 2
        sigma_hat_g = output_g[:, :, 2:].reshape((batch_size, self.decoding_steps, 2, 2)) # B X T X 2 X 2
        ...
        # import pdb
        # pdb.set_trace()
        x = sigma.matmul(z.reshape(batch_size, self.decoding_steps, 2).unsqueeze(-1)).squeeze(-1) + mu
        x_g = sigma_g.matmul(z.reshape(batch_size, self.decoding_steps, 2).unsqueeze(-1)).squeeze(-1) + mu_g
        ...
        return x, mu, sigma, (x_g, mu_g, sigma_g)