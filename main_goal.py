...
from Goals.models import Global_Scene_CAM_Goal_NFDecoder, Only_Global_Scene_CAM_Goal_NFDecoder, Global_Scene_CAM_Path_NFDecoder
from Goals.utils import ModelTrainer, ModelTest
...
def train(args):
    ...
    # 2) define model
    if args.model_type == 'AttGlobal_Scene_CAM_Goal_NFDecoder' or args.model_type == 'Only_AttGlobal_Scene_CAM_Goal_NFDecoder':
        crossmodal_attention = True
    else:
        crossmodal_attention = False

    if args.model_type == 'AttGlobal_Scene_CAM_Goal_NFDecoder':
        model = Global_Scene_CAM_Goal_NFDecoder(device=device, agent_embed_dim=args.agent_embed_dim, nfuture=nfuture,
                                        att_dropout=args.att_dropout,
                                        velocity_const=args.velocity_const, num_candidates=args.num_candidates,
                                        att=crossmodal_attention)
    elif args.model_type == 'Only_AttGlobal_Scene_CAM_Goal_NFDecoder':
        model = Only_Global_Scene_CAM_Goal_NFDecoder(device=device, agent_embed_dim=args.agent_embed_dim, nfuture=nfuture,
                                att_dropout=args.att_dropout,
                                velocity_const=args.velocity_const, num_candidates=args.num_candidates,
                                att=crossmodal_attention)
...
def test(args):
    ...
    if args.model_type == 'Global_Scene_CAM_Goal_NFDecoder' or args.model_type == 'AttGlobal_Scene_CAM_Goal_NFDecoder' or args.model_type == 'Only_AttGlobal_Scene_CAM_Goal_NFDecoder':
        if args.model_type == 'AttGlobal_Scene_CAM_Goal_NFDecoder' or args.model_type == 'Only_AttGlobal_Scene_CAM_Goal_NFDecoder':
            crossmodal_attention = True
        else:
            crossmodal_attention = False
