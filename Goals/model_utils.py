...
class CrossModalPathDecoder(nn.Module):
    ...
    Dynamic Decoder for R2P2 RNN
    """
    def __init__(self, image_dim=32, hidden_size=150, context_dim=50, decoding_steps=5, velocity_const=0.5, att=True, goal_enc_dim=200):
        ...
        self.mlp_path = nn.Sequential(
            nn.Linear(2*2, 50),
            nn.ReLU(),
            nn.Linear(50, hidden_size * decoding_steps) # DON'T USE ACTIVATION AT THE TOP MOST LAYER
        )
        self.crossmodal_attention = CrossModalAttention(encoder_dim=image_dim, decoder_dim=hidden_size, attention_dim=hidden_size, att=att) # map2agent attention
        self.mlp = nn.Sequential(
            nn.Linear(hidden_size+context_dim+image_dim+goal_enc_dim, 50),
            nn.Softplus(),
            nn.Linear(50, 50),
            nn.Tanh(),
            nn.Linear(50, 6) # DON'T USE ACTIVATION AT THE TOP MOST LAYER
    ...
    def infer(self, x, static, init_velocity, init_position, global_scene_encoding, episode_idx, goal_encoding):
        ...
        x_prev = torch.cat((init_position.unsqueeze(1), x_goal), dim=1) # B X 2 X 2

        # Build the flattend & zero padded previous states for GRU input
        x_flat = x_prev.reshape((batch_size, -1)) # B X 2 X 2 >> B X (2*2)

        ##########################################
        # MLP for "PATH" generation 
        ##########################################
        dynamic_encoding = self.mlp_path(x_flat) # dynamic_encoding: B X (150 * (T-1)) (hidden size)

        dynamic_encoding_ = dynamic_encoding # Since T is 1(only for 1 goal)
        dynamic_encoding = dynamic_encoding.reshape(batch_size, T-1, -1) # B X T X 150
        episode_idx_ = episode_idx.repeat_interleave(T)
        goal_encoding = goal_encoding.repeat_interleave(T)

        att_scenes, alpha = self.crossmodal_attention(global_scene_encoding, dynamic_encoding_, episode_idx_)
        att_scenes = att_scenes.reshape(batch_size, T, -1)

        # Concat the dynamic and static encodings
        dynamic_static = torch.cat((dynamic_encoding, static, att_scenes, goal_encoding), dim=-1) # B X T X (200+200)
    ...
    def forward(self, z, x_flat, static, dx, x_prev, global_scene_encoding, episode_idx, goal, goal_encoding):
        ...
        # Concat the dynamic and static encodings
        dynamic_static = torch.cat((dynamic_encoding, static, att_scene, goal_encoding), dim=-1) # B X 200
