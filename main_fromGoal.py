...
from FromGoal.models import Global_Scene_CAM_Goal_NFDecoder, Only_Global_Scene_CAM_Goal_NFDecoder, Global_Scene_CAM_Path_NFDecoder
from FromGoal.utils import ModelTrainer, ModelTest
        ...
        with torch.no_grad():
            ...
            for b, batch in tqdm(enumerate(dataloader), total=len(dataloader) // batch_size, desc='predict'):
                ...
                for candi in range(gen_goals.size(1)):
                    ...
                    # Prior Loss (p loss)
                    gen_trajs, z, mu, sigma, gen_trajs_goal = self.path_model(motion_encoding_, src_lens, agent_tgt_three_mask,
                                                                episode_idx, decode_start_vel, decode_start_pos,
                                                                num_src_trajs, scene_encoding_, gen_goals[:,candi].squeeze(1), goal_encoding)
                    # gen_trajs = gen_trajs_goal
