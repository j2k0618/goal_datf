...
class ModelTrainer:
    ...
    def train_single_epoch(self):
        ...
        for b, batch in enumerate(self.train_loader):
            ...
            # Prior Loss (p loss)
            gen_trajs = self.path_model(motion_encoding_, past_agents_traj_len, agent_tgt_three_mask,
                                                episode_idx, decode_start_vel, decode_start_pos,
            ...
            if self.beta != 0.0:
                if self.path_ploss_type == 'mseloss':
                    # ploss = self.ploss_criterion(gen_trajs, past_agents_traj)
                    ploss = self.path_ploss_criterion(gen_trajs, future_agents_traj[:,:-1,:])
                else:
            ...
            batch_loss += (self.beta * batch_path_ploss) * self.path_weight
